...
Core redesign: ``IndexParts`` carries ``global_snippets`` / ``cross_ref_hints``
and the ``global_count`` / ``project_count`` split (by the controlled
ScopeDecision), replacing the old preference/project_fact/idea_seed fields.
"""
...
def test_render_keeps_summary_and_delimiters_under_tiny_budget() -> None:
    parts = IndexParts(
        global_snippets=_many("pref", 10),
        cross_ref_hints=["idea about an async runtime cli tool"],
...
def test_generous_budget_keeps_everything() -> None:
    parts = IndexParts(
        global_snippets=_many("pref", 3),
        cross_ref_hints=["idea one", "idea two"],
        entity_tags=["rust"],
        global_count=3,
        project_count=1,
    )
...
def test_cross_ref_hints_dropped_only_after_snippets() -> None:
    parts = IndexParts(
        global_snippets=_many("pref", 6),
        cross_ref_hints=["idea seed hint one here", "idea seed hint two here"],
        entity_tags=["rust", "async"],
        global_count=6,
        project_count=0,
    )
    # Budget tight enough to drop most snippets but (ideally) keep at least the
    # summary; verify snippets go before cross-ref hints where both cannot fit.
    text, est = render_index(parts, token_budget=45)
    assert est <= 45
    # If any cross-ref hint survived, then snippets must have been pruned first:
    # i.e. we should not see all 6 snippets while a hint was dropped.
